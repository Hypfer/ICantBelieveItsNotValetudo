    ...
    switch (type) {
        ...
        case RRMapParser.TYPES.CHARGER_LOCATION:
            result[type] = {
                ...
                angle: length >= 12 ? buf.readInt32LE(0x10 + offset) : 0
            };
            break;
        case RRMapParser.TYPES.IMAGE:
            if (hlength > 24) { // thank you roborock for not using map version field for this! /* sarcasm */
                g3offset = 4;
            }
            const parameters = {
                segments: {
            ...
            if(parameters.dimensions.height > 0 && parameters.dimensions.width > 0) {
                for (let s, i = 0; i < length; i++) {
                    const val = buf.readUInt8(0x18 + g3offset + offset + i);
                    if ((val & 0x07) === 0) continue;
                    let coords = [
                    ...
                    if ((val & 0x07) === 1) {
                        ...
                        s = (val & 248) >> 3;
                        if (s !== 0) {
                            if (!parameters.pixels.segments[s]) parameters.pixels.segments[s] = [];
                            parameters.pixels.segments[s].push(coords);
                        }
                    }
                }
            }
