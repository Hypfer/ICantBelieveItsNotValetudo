...
class MapDrawer {
    ...
    async processDrawnMap(mapBuffer, bounds) {
        ...
        if (this.settings.overlay_path) {
            const overlayImage = Jimp.read(this.settings.overlay_path);
        ...
        if (this.settings.overlay_path) {
            const underlayImage = Jimp.read(this.settings.underlay_path);

            underlayImage.scale(this.settings.underlay_path, Jimp.AUTO);
