...
class MapDrawer {
    ...
    cropCanvas (sourceCanvas,left,top,width,height) {
        let destCanvas = Canvas.createCanvas(width, height);
    ...
    draw(skipLayers) {
        ...
        const cropWidth = canvasWidth * this.settings.scale - this.settings.crop_left - this.settings.crop_right;
        const cropHeight = canvasHeight * this.settings.scale - this.settings.crop_top - this.settings.crop_bottom;

        const croppedMapCanvas = this.cropCanvas(mapCanvas, this.settings.crop_left, this.settings.crop_top, cropWidth, cropHeight);

        return croppedMapCanvas.toBuffer();
    }
}
