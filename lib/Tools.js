...
const Tools = {
    ...
    DRAW_MAP_PNG: function (options, callback) {
        ...
        const COLORS = {
            ...
            obstacle: Jimp.cssColorToHex('#a1dbff'),
            ...
            if (!err) {
                ...
                ["floor", "obstacle"].forEach(key => {
                    ...
                    options.parsedMapData.image.pixels[key].forEach(function drawPixel(px) {
                        image.setPixelColor(color, (px % options.parsedMapData.image.dimensions.width), (options.parsedMapData.image.dimensions.height - 1 - Math.floor(px / options.parsedMapData.image.dimensions.width)));
                    });
                });
                ...
                if (settings.drawCurrentlyCleanedZones && options.parsedMapData.currently_cleaned_zones) {
                    let currentlyCleanedZones = options.parsedMapData.currently_cleaned_zones.map(zone => {
                        return [[zone[0],zone[1]],[zone[2],zone[3]]].map(point => {
                            return [
                                Math.floor(point[0]/50 - options.parsedMapData.image.position.left),
                                Math.floor(point[1]/50 - options.parsedMapData.image.position.top)
                            ]});
                    });
                ...
                if (settings.drawCurrentlyCleanedBlocks && options.parsedMapData.currently_cleaned_blocks) {
                    let i, j, resultColor, overlayColor = Jimp.intToRGBA(COLORS['cleaned_block']);
                    options.parsedMapData.currently_cleaned_blocks.forEach(segnum => {
                        ...
                        options.parsedMapData.image.pixels.segments[segnum].forEach(px => {
                            i = px % options.parsedMapData.image.dimensions.width;
                            j = options.parsedMapData.image.dimensions.height - 1 - Math.floor(px / options.parsedMapData.image.dimensions.width);
                            resultColor = calcOverlayColor(Jimp.intToRGBA(image.getPixelColor(i,j)),overlayColor);
                            image.setPixelColor(Jimp.rgbaToInt(resultColor.r,resultColor.g,resultColor.b,255),i,j);
                        });
                    });
                }
                ...
                Promise.all([loadChargerImage, loadRobotImage]).then(() => {
                    ...
                    if (settings.autoCrop) {
                        image.autocrop({ leaveBorder: parseInt(settings.autoCrop) || 20 });
                    }
